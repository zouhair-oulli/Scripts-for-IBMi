...
The objective of these programs is to run a check for SSH errors in QSYSOPR and to verify whether the SSHD server is active. Depending on the result, the program should either inform the user that the SSH service is running or restart it.
...
IBM i: `STRSQL à SELECT count(*) FROM QSYS2/NETSTAT_INFO WHERE LOCAL_PORT ='X' and REMOTE_PORT != '0'`  
ACS: `SELECT count(*) FROM QSYS2.NETSTAT_INFO WHERE LOCAL_PORT ='X' and REMOTE_PORT != '0'`

💡 Check the port number and IP addresses of remote connections at local port X
`SELECT * FROM QSYS2.NETSTAT_INFO WHERE LOCAL_PORT ='X' and REMOTE_PORT != '0';`
